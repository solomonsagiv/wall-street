...
public class FullSettingOptionsProps extends MyGuiComps.MyPanel {
        ...
        submitBtn.addActionListener(new ActionListener() {
            ...
            public void actionPerformed(ActionEvent actionEvent) {
                switch (FullSettingOptionsPanel.optionsCombo.getSelectedItem().toString()) {
                    case "week":
                        expName = ExpStrings.week;
                        break;
                    case "month":
                        expName = ExpStrings.month;
                        break;
                    case "e1":
                        expName = ExpStrings.e1;
                        break;
                    case "e2":
                        expName = ExpStrings.e2;
                        break;
                    default:
                        expName = ExpStrings.week;
                        break;
                ...
                if (FullSettingOptionsPanel.clientsCombo.getSelectedItem() == "STOCKS") {
                    for (BASE_CLIENT_OBJECT client : LocalHandler.clients) {
                        if (client instanceof STOCK_OBJECT) {
                            ...
                            // Update client
                            updateOptionsData(client, expName);
    ...
    private void updateOptionsData(BASE_CLIENT_OBJECT client, String expName) {
        try {
            Options options = client.getExps().getExp(expName).getOptions();
            // Interest
