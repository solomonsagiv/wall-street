...
public class IndRacesChart extends MyChartCreator {

    // Constructor
    public IndRacesChart( BASE_CLIENT_OBJECT client) {
        super(client);
    }
    ...
    public void createChart() throws CloneNotSupportedException {
        ...
        // Delta
        MyTimeSeries indRacesSerie = client.getLogicService().getIndRacesSerie();
        indRacesSerie.setColor(Themes.PURPLE);
        indRacesSerie.setStokeSize(1.5f);

        series = new MyTimeSeries[1];
        series[0] = indRacesSerie;

        // Chart
        MyChart indRacesChart = new MyChart(client, series, props1);

        MyProps newProps = (MyProps) props.clone();
        newProps.setProp(ChartPropsEnum.INCLUDE_DOMAIN_AXIS, false);

        // ----- Charts ----- //
        MyChart[] charts = {indRacesChart};
