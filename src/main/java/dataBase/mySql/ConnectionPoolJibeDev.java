...
public class ConnectionPoolJibeDev implements IConnectionPool {

    public static void main(String[] args) throws ClassNotFoundException, SQLException {
        Connection jibe_dev_conn = ConnectionPoolJibeDev.get_jibe_dev_single_connection();
    ...
    // Instance
    private static ConnectionPoolJibeDev connectionPoolJibeProd;
    private String url;
    private String user;
    ...
    // Constructor
    private ConnectionPoolJibeDev(String url, String user, String password, List<Connection> connections) {
        this.url = url;
        this.user = user;
    ...
    private ConnectionPoolJibeDev() {
    }

    public static ConnectionPoolJibeDev getInstance() {
        if (connectionPoolJibeProd == null) {
            connectionPoolJibeProd = new ConnectionPoolJibeDev();
        }
        return connectionPoolJibeProd;
    }

    public static ConnectionPoolJibeDev getConnectionsPoolInstance(int connection_count) {
        if (connectionPoolJibeProd == null) {
            try {
                ...
                connectionPoolJibeProd = ConnectionPoolJibeDev.create(connectionType, connection_count);
            } catch (Exception e) {
    ...
    public static ConnectionPoolJibeDev getConnectionsPoolInstance() {
        if (connectionPoolJibeProd == null) {
            try {
                MyDBConnections dbConnections = new MyDBConnections();
                DBConnectionType connectionType = dbConnections.getConnectionType(MyDBConnections.JIBE_POSTGRES);
                connectionPoolJibeProd = ConnectionPoolJibeDev.create(connectionType, Manifest.POOL_SIZE);
            } catch (Exception e) {
                Arik.getInstance().sendMessage(e.getMessage() + "\n" + e.getCause());
            }
        }
    ...
    public static ConnectionPoolJibeDev create(DBConnectionType dbConnectionType, int connection_count) throws SQLException {
        ...
        try {
            ...
            return new ConnectionPoolJibeDev(dbConnectionType.getUrl(), dbConnectionType.getUser(), dbConnectionType.getPassword(), pool);
        }
    }
